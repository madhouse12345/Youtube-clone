...
const Playvideo = () => {
  ...
  const fetchVideoDetails = async () => {
    ...
    await fetch(videoDetails_url).then(res=>res.json()).then(data => seApiData(data.items[0]))
  }
  ...
  const comments_url =`https://www.googleapis.com/youtube/v3/commentThreads?part=snippet%2Creplies&videoId=${videoId}&key=${API_KEY}`;
  ...
  return (
    ...
    <p>{apiData?value_converter(apiData.statistics.viewCount):"16K"} views  &bull;  {apiData?moment(apiData.snippet.publishedAt).fromNow():"3 days ago"}</p>
    <div>
